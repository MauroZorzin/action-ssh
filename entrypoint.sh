...
for host in ${INPUT_HOSTS}; do
    ...
    sh -c "ssh -q -t -i ~/.ssh/id_rsa -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o ServerAliveInterval=1000 -o ClientAliveInterval=1000 '${host%%:*}' -p ${PORT:-22} < ~/script.sh"
    echo ""
done
